...
DEFAULT_OUT = Path.home() / "Documents" / "HMM-Discovery-AllDB-Benchmark"
...
class Benchmark:
    ...
    def run_pfam_hmmscan(self, db: dict, input_faa: Path) -> dict:
        ...
        if not hmm.exists():
            if is_tar_gz:
                with tarfile.open(cache_file, "r:gz") as tf, hmm.open("wb") as out_hmm:
                    hmm_members = [
                        member for member in tf.getmembers()
                        if member.isfile() and member.name.lower().endswith(".hmm")
                    ]
                    if not hmm_members:
                        raise RuntimeError(f"No HMM files found in {cache_file}")
